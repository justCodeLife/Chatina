...
const room_users_modal_body = document.getElementById('room_users_modal_body')
...
socket.on('roomOnlineUsers', (room_users) => {
    for (const usr of room_users) {
        const el = document.createElement('tr')
        el.setAttribute('id', usr.user_id)
        if (usr.user_id !== user._id) {
            el.innerHTML = `
        <td>${usr.name || 'ناشناس'}</td>
         <td>
             <i class="btn fa fa-close text-danger mx-1" onclick="kickUserFromRoom(room, '${usr.user_id}')"></i>
         </td>`
        } else {
            el.innerHTML = `
         <td>${usr.name || 'ناشناس'}</td>
        ...
        room_users_modal_body.append(el)
    }
})
socket.on('deleteRemovedUserElement', ({user_id}) => {
    document.getElementById(user_id).remove()
})
socket.on('addJoinedUserElement', ({user}) => {
    const el = document.createElement('tr')
    el.setAttribute('id', user.user_id)
    el.innerHTML = `
         <td>${user.name || 'ناشناس'}</td>
          <td>
              <i class="btn fa fa-close text-danger mx-1" onclick="kickUserFromRoom(room, '${user.user_id}')"></i>
         </td>`
    room_users_modal_body.append(el)
})
...
function kickUserFromRoom(room, user_id) {
    socket.emit('kickUserFromRoom', {room: room.slug, user_id})
}
